...
import { useMedicionesWS } from "../../hook/useMedicionesWebSocket";

import { useMediciones, useMedicionesId } from "../../hook/useMediciones";
...
export default function MedicionesSection() {
    ...
    // Socket
    const [medicionesTiempoReal, setMedicionesTiempoReal] = useState<Mediciones[]>([]);
    const [dispositivoAnterior, setDispositivoAnterior] = useState<string | null>(null);

    const handleNuevaMedicion = (nuevaMedicion: Mediciones) => {
        setMedicionesTiempoReal((prev) => {
            const updated = [...prev, nuevaMedicion];
            if (updated.length > 20) updated.shift();
            return updated;
        });
    };

    // Reset cache si se cambia de dispositivo
    const resetCache = () => {
        setMedicionesTiempoReal([]);
    };

    useEffect(() => {
        if (dispositivoAnterior !== dispositivoSeleccionado) {
            setDispositivoAnterior(dispositivoSeleccionado);
        }
    }, [dispositivoSeleccionado]);

    useMedicionesWS(dispositivoSeleccionado, handleNuevaMedicion, resetCache);

    console.log("ultimas mediciones: ", medicionesTiempoReal);
